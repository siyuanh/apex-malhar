...
{
  public static class MinNumGen extends BaseOperator implements InputOperator
  {
  ...
  @Test
  public void MinTestWithStream()
  {
    MinNumGen minNumGen = new MinNumGen();
    Collector collector = new Collector();
    ApexStream<Integer> stream = StreamFactory.fromInput(minNumGen, minNumGen.output);
    stream.window(new WindowOption.GlobalWindow(), new TriggerOption().withEarlyFiringsAtEvery(Duration.millis(500)))
        .accumulate(new Min<Integer>()).endWith(collector, collector.input).runEmbedded(false, 600000, new Callable<Boolean>()
        {
          @Override
          public Boolean call() throws Exception
          {
            return MinNumGen.count > 35;
          }
        });
    
    Assert.assertEquals(-2, collector.getMin());
