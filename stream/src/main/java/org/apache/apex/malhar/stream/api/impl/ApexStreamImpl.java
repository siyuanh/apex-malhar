...
public class ApexStreamImpl<T> implements ApexStream<T>
{
  ...
  public <STREAM extends ApexStream<T>> STREAM filter(String name, final Function.FilterFunction<T> filter)
  {
    ...
    return (STREAM)addOperator(name, filterFunctionOperator, (DefaultInputPort<T>)filterFunctionOperator.in, (DefaultOutputPort<T>)filterFunctionOperator.out);
  }

  @Override
  public <STREAM extends ApexStream<T>> STREAM reduce(Function.ReduceFunction reduce)
  {
    return reduce(reduce.toString(), reduce);
  }

  @Override
  public <STREAM extends ApexStream<T>> STREAM reduce(String name, Function.ReduceFunction reduce)
  {
  ...
  @Override
  public <O, STREAM extends ApexStream<O>> STREAM fold(final O initialValue, Function.FoldFunction fold)
  {
    return fold(fold.toString(), initialValue, fold);
  }

  @Override
  public <O, STREAM extends ApexStream<O>> STREAM fold(String name, O initialValue, Function.FoldFunction fold)
  {
  ...
  public <STREAM extends ApexStream<Map<Object, Integer>>> STREAM countByKey()
  {
    ...
    return (STREAM)addOperator("CounterByKey", uniqueCounter, (Operator.InputPort<T>)uniqueCounter.data, uniqueCounter.count);
  }
  ...
  public <O, STREAM extends ApexStream<O>> STREAM addOperator(String opName, Operator op, Operator.InputPort<T> inputPort, Operator.OutputPort<O> outputPort)
  {
    ...
    if (delegator != null) {
      delegator.addOperator(opName, op, inputPort, outputPort);
    }
  ...
  @Override
  public void runEmbedded(boolean async)
  {
