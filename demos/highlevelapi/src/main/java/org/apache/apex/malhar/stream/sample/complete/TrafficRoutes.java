...
import org.apache.apex.malhar.stream.api.impl.accumulation.Group;
...
public class TrafficRoutes implements StreamingApplication
{
  ...
  static class TrackSpeed extends
      CompositeStreamTransform<KeyValPair<String, StationSpeed>, OutputPojo>
  {
    @Override
    public ApexStream<OutputPojo> compose(ApexStream<KeyValPair<String, StationSpeed>> inputStream)
    {
      // Apply a GroupByKey transform to collect a list of all station
      // readings for a given route.
      ApexStream<Tuple.WindowedTuple<KeyValPair<String, List<StationSpeed>>>> timeGroup =
          ((WindowedStream<KeyValPair<String,StationSpeed>>)inputStream)
      ...
      ApexStream<Tuple.TimestampedTuple<KeyValPair<String, RouteInfo>>> stats = timeGroup
          .flatMap(new GatherStats(), name("GatherStats"));
      
      // Format the results for writing to JDBC table.
      ApexStream<OutputPojo> results = stats.map(new FormatStatsFn(), name("FormatStatsFn"));
