...
public class TwitterAutoComplete implements StreamingApplication
{
      ...
      extends CompositeStreamTransform<CompletionCandidate, Tuple.WindowedTuple<KeyValPair<String, List<CompletionCandidate>>>>
  {
        ...
        ApexStream<CompletionCandidate> input)
    {
      ...
      return ((WindowedStream<KeyValPair<String,CompletionCandidate>>)input
        .flatMap("Extract Prefixes", new AllPrefixes(minPrefix, 3)))
        .accumulateByKey("TopNByKey", topNByKey, new Function.MapFunction<KeyValPair<String, CompletionCandidate>, Tuple<KeyValPair<String,
          CompletionCandidate>>>()
        {
          @Override
      ...
      extends CompositeStreamTransform<String, Tuple.WindowedTuple<KeyValPair<String, List<CompletionCandidate>>>>
  {
    ...
    public ApexStream<Tuple.WindowedTuple<KeyValPair<String, List<CompletionCandidate>>>> compose(ApexStream<String> inputStream)
    {
      ...
      ApexStream<CompletionCandidate> candidates = ((WindowedStream<String>)inputStream)
          .countByKey("Hashtag Count", new Function.MapFunction<String, Tuple<KeyValPair<String, Long>>>()
          {
            @Override
          ...
          }).map("KeyValPair to CompletionCandidate", new Function.MapFunction<Tuple.WindowedTuple<KeyValPair<String,Long>>, CompletionCandidate>()
          {
            @Override
  ...
  public void populateDAG(DAG dag, Configuration conf)
  {
    ...
    ApexStream<String> tags = StreamFactory.fromInput("tweetSampler", input, input.text)
        .filter("ACSII Filter", new ASCIIFilter())
        .flatMap("Extract Hashtags", new ExtractHashtags());
